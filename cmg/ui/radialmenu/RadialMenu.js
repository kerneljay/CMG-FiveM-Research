...
function RadialMenu(params) {
    ...
    self.pageSize  = params.pageSize > 0 ? params.pageSize : 0;
...
RadialMenu.prototype.open = function () {
    var self = this;
    if (!self.currentMenu) {
        self.allMenuItems = self.menuItems;
        self.currentPage  = 0;
        var levelItems = self.pageSize > 0 ? self.getPageItems() : self.menuItems;
        self.currentMenu = self.createMenu('menu inner', levelItems, false, true);
...
RadialMenu.prototype.getPageItems = function () {
    var self = this;
    var start = self.currentPage * self.pageSize;
    var slice = self.allMenuItems.slice(start, start + self.pageSize);
    if (self.allMenuItems.length > start + self.pageSize) {
        slice.push({ id: '__nextPage__', title: 'More', icon: '#more' });
    }
    return slice;
};

RadialMenu.prototype.refreshPage = function () {
    var self = this;
    if (self.parentMenu.length > 0) return;
    if (!self.currentMenu) return;
    self.currentMenu.remove();
    self.currentMenu = self.createMenu('menu inner', self.getPageItems(), false, true);
    self.holder.appendChild(self.currentMenu);
    RadialMenu.nextTick(function () {
        self.currentMenu.setAttribute('class', 'menu');
    });
...
RadialMenu.prototype.showNestedMenu = function (item) {
    ...
    self.currentMenu = self.createMenu('menu inner', item.items, true, false);
    self.holder.appendChild(self.currentMenu);
...
RadialMenu.prototype.handleCenterClick = function () {
    ...
    } else if (self.pageSize > 0 && self.currentPage > 0) {
        self.currentPage--;
        self.refreshPage();
    } else {
        self.close();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
RadialMenu.prototype.createMenu = function (classValue, levelItems, nested, isRootWithPagination) {
    var self = this;
    ...
    var showBack = nested || (isRootWithPagination && self.currentPage > 0);
    if (showBack) {
...
RadialMenu.prototype.appendSectorPath = function (startAngleDeg, endAngleDeg, svg, item, index) {
    ...
    if (item) {
        ...
        if (item.iconNumber !== undefined) {
            var number = self.createIconNumber(centerPoint.x, centerPoint.y, item.iconNumber);
            if (item.title) {
                number.setAttribute('transform', 'translate(-5,-8)');
            } else {
                number.setAttribute('transform', 'translate(-5,-5)');
            }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
RadialMenu.prototype.createIconNumber = function (x, y, number) {
    var self = this;
    var text = document.createElementNS('http://www.w3.org/2000/svg', 'text');
    text.setAttribute('text-anchor', 'middle');
    text.setAttribute('x', RadialMenu.numberToString(x));
