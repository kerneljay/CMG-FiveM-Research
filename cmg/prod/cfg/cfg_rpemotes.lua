--[[
    LEVEL 1 BEGINNER GUIDE — Rpemotes
    ======================================

    File: cmg/prod/cfg/cfg_rpemotes.lua
    ...
    Quick map of this file (automatic static scan):
      ...
      * Modules/config loaded: cfg/cfg_pazeee_emotes
...
-- === HELPER FUNCTION: Translate(message) ===
local function Translate(message)
    local pack = Config.Languages[Config.MenuLanguage]
    if type(pack) == "table" then
        return pack[message] or ""
    end
    return ""
end
...
---------- END OF SCENARIOS --------------
    ...
    -- Pazeee fireworks (stream: fireworks_emotes)
    ["pfireworksu"] = {
...
do
    local escrowPazeee = CMG.loadModule("cfg/cfg_pazeee_emotes")
    for name, info in pairs(escrowPazeee.shared or {}) do
        RP.Shared[name] = info
    end
    for name, info in pairs(escrowPazeee.emotes or {}) do
        -- Carry pack entries are two-player sync pairs (partner in [4], Attachto on one side); keep in Shared like RPEmotes shared emotes.
        if string.sub(name, 1, 6) == "pcarry" then
            RP.Shared[name] = info
        else
            RP.Emotes[name] = info
        end
    end
    for name, info in pairs(escrowPazeee.propEmotes or {}) do
        RP.PropEmotes[name] = info
    end
    RP.escrowPazeeeKeys = escrowPazeee.keys
end
