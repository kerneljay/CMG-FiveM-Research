...
cfg.emotes = {
    ...
    ["inspect"] = {"random@train_tracks", "idle_e", "Inspect", animationOptions =
    {
        allowedPrison = true,
    }},
    ...
    -----------------------------------------------------------------------------------------------------------
        ...
        ["cutthroat"] = {"anim@mp_player_intcelebrationmale@cut_throat", "cut_throat", "Cut Throat", animationOptions =
        {
            allowedPrison = true,
        }},
        ["cutthroat2"] = {"anim@mp_player_intcelebrationfemale@cut_throat", "cut_throat", "Cut Throat 2", animationOptions =
        {
            allowedPrison = true,
        }},
...
cfg.props = {
    ...
    ["tablet2"] = {"amb@code_human_in_bus_passenger_idles@female@tablet@idle_a", "idle_a", "Tablet 2", animationOptions =
    {
        prop = "prop_cs_tablet",
        propBone = 28422,
        propPlacement = {-0.05, 0.0, 0.0, 0.0, -90.0, 0.0},
        emoteLoop = true,
        emoteMoving = true,
    }},
...
cfg.guns = {
    ...
    -- Sinner
    ["batonhold"] = {"export@batonhold", "head_000_r", "Baton Hold", animationOptions =
    {
        emoteLoop = true,
        emoteMoving = true
    }},
    ["holdingarms"] = {"export@holdingarms", "head_000_r", "Holding Arms", animationOptions =
    {
        emoteLoop = true,
        emoteMoving = true
    }},
    ["holdingvest3"] = {"export@holdingvest", "head_001_r", "Holding Vest 3", animationOptions =
    {
        emoteLoop = true,
        emoteMoving = true
    }},
    ["taserback"] = {"export@taserback", "head_000_r", "Taser Back", animationOptions =
    {
        emoteLoop = true,
        emoteMoving = true
    }},
    ["stopvehicle"] = {"export@stopvehicle", "head_000_r", "Stop Vehicle", animationOptions =
    {
        emoteLoop = true,
        emoteMoving = true
    }},
...
-- Known buggy emotes or that cause confusion
local explictBannedLists = {
    ["lollipop3a"] = true,
    ["patrolcar"] = true,
    ["patrolcarl"] = true,
    ["patrolcarr"] = true,
...
for name, info in pairs(cfgRP.Expressions) do
    if not cfg.moods[name] then
        cfg.moods[name] = {"Expression", info[1]}
    end
end

for name, emoteInfo in pairs(cfgRP.Walks) do
    if not cfg.walks[name] then
        cfg.walks[name] = {emoteInfo[1]}
    end
end
...
local function canModelNameBeUsed(name)
    ...
    for _, unwantedName in pairs(unwantedEmoteNames) do
        if string.match(lowerName, unwantedName) then
            return false
        end
    end
    return true
...
local function canExternalEmoteBeUsed(emoteInfo)
    ...
    if emoteInfo.AnimationOptions then
        if emoteInfo.AnimationOptions.Prop then
            local propName = emoteInfo.AnimationOptions.Prop
            if not isModelValid(GetHashKey(propName)) or not canModelNameBeUsed(propName) then
                return false
            end
        end
        if emoteInfo.AnimationOptions.SecondProp then
            local propName = emoteInfo.AnimationOptions.SecondProp
            if not isModelValid(GetHashKey(propName)) or not canModelNameBeUsed(propName) then
                return false
            end
        end
    end
    if emoteInfo.AnimalEmote then
        return false
    end
    return true
end
...
local function canExternalAnimalEmoteBeUsed(emoteInfo)
    ...
    if emoteInfo.AnimationOptions then
        if emoteInfo.AnimationOptions.Prop then
            if not isModelValid(GetHashKey(emoteInfo.AnimationOptions.Prop)) or not canModelNameBeUsed(emoteInfo.AnimationOptions.Prop) then
                return false
            end
        end
        if emoteInfo.AnimationOptions.SecondProp then
            if not isModelValid(GetHashKey(emoteInfo.AnimationOptions.SecondProp)) or not canModelNameBeUsed(emoteInfo.AnimationOptions.SecondProp) then
                return false
            end
        end
    end
...
local function copyPropTextureVariations(src)
    ...
    for i, entry in ipairs(src) do
        out[i] = {
            ...
            value = entry.Value or entry.value,
        }
    end
    return out
end
...
local function copyExternalEmote(emoteInfo)
    ...
    if opt then
        newEmoteInfo.animationOptions = {
            emoteLoop = opt.EmoteLoop,
            emoteMoving = opt.EmoteMoving,
            emoteStuck = opt.EmoteStuck,
            emoteDuration = opt.EmoteDuration,
            onFootFlag = opt.onFootFlag,
            animFlag = opt.Flag,
            fullBody = opt.FullBody,
            vehicleRequirement = opt.vehicleRequirement,
            notInVehicle = opt.NotInVehicle,
            onlyInVehicle = opt.onlyInVehicle,
            syncOffsetFront = opt.SyncOffsetFront,
            syncOffsetSide = opt.SyncOffsetSide,
            syncOffsetHeight = opt.SyncOffsetHeight,
            syncOffsetHeading = opt.SyncOffsetHeading,
            prop = opt.Prop,
            propBone = opt.PropBone,
            propPlacement = opt.PropPlacement,
            propNoCollision = opt.PropNoCollision,
            secondProp = opt.SecondProp,
            secondPropBone = opt.SecondPropBone,
            secondPropPlacement = opt.SecondPropPlacement,
            secondPropNoCollision = opt.SecondPropNoCollision,
            propTextureVariations = copyPropTextureVariations(opt.PropTextureVariations),
            startDelay = opt.StartDelay,
            ...
            pos = opt.pos,
            rot = opt.rot,
            ptfxAsset = opt.PtfxAsset,
            ptfxName = opt.PtfxName,
            ptfxNoProp = opt.PtfxNoProp,
            ptfxPlacement = opt.PtfxPlacement,
            ptfxInfo = opt.PtfxInfo,
            ptfxWait = opt.PtfxWait,
            ptfxCanHold = opt.PtfxCanHold,
            ptfxBone = opt.PtfxBone,
            ptfxColor = opt.PtfxColor,
            exitEmote = opt.ExitEmote,
            exitEmoteType = opt.ExitEmoteType,
            blendInSpeed = opt.BlendInSpeed,
            blendOutSpeed = opt.BlendOutSpeed,
            invisible = opt.Invisible,
        }
    end
    if emoteInfo.AnimalEmote then
        newEmoteInfo.animalEmote = true
    end
    if emoteInfo.AdultAnimation then
        newEmoteInfo.adultAnimation = true
    end
    if emoteInfo.devOnly == true or emoteInfo.DevOnly == true then
        newEmoteInfo.devOnly = true
    end
    return newEmoteInfo
end
...
local function stripUnwantedEmotes(emoteTable)
    for emoteId, emoteInfo in pairs(emoteTable) do
        if not canModelNameBeUsed(emoteInfo[3]) then
            emoteTable[emoteId] = nil
        end
    end
end

stripUnwantedEmotes(cfg.shared)
stripUnwantedEmotes(cfg.dances)
stripUnwantedEmotes(cfg.emotes)
stripUnwantedEmotes(cfg.props)

for name, info in pairs(cfgRP.Shared) do
    if not doesEmoteWithNameExist(name) and canExternalEmoteBeUsed(info) and canEmoteNameBeUsed(name) then
        cfg.shared[name] = copyExternalEmote(info)
    end
end

for name, info in pairs(cfgRP.Dances) do
    if not doesEmoteWithNameExist(name) and canExternalEmoteBeUsed(info)and canEmoteNameBeUsed(name) then
        cfg.dances[name] = copyExternalEmote(info)
    end
end

for name, info in pairs(cfgRP.Emotes) do
    if not doesEmoteWithNameExist(name) and canExternalEmoteBeUsed(info) and canEmoteNameBeUsed(name) then
        cfg.emotes[name] = copyExternalEmote(info)
    end
end

for name, info in pairs(cfgRP.PropEmotes) do
    if not doesEmoteWithNameExist(name) and canExternalEmoteBeUsed(info) and canEmoteNameBeUsed(name) then
        cfg.props[name] = copyExternalEmote(info)
    end
end

for name, info in pairs(cfgRP.AnimalEmotes) do
    if not doesEmoteWithNameExist(name) and canExternalAnimalEmoteBeUsed(info) and canEmoteNameBeUsed(name) then
        local animalEntry = copyExternalEmote(info)
        animalEntry.animalEmote = true
        cfg.animals[name] = animalEntry
    end
end

stripUnwantedEmotes(cfg.animals)

for name, info in pairs(cfgRP.Exits) do
    if not cfg.exits[name] and canExternalEmoteBeUsed(info) and canEmoteNameBeUsed(name) then
        cfg.exits[name] = copyExternalEmote(info)
    end
end

cfg.fireworksEmotes = {
    "pfireworksu", "pfireworksv", "pfireworksw", "pfireworksx", "pfireworksz", "pfireworksza",
    "pfireworksa", "pfireworksb", "pfireworksc", "pfireworksd", "pfireworkse", "pfireworksf",
    "pfireworksg", "pfireworksh", "pfireworksi", "pfireworksj", "pfireworksk", "pfireworksl",
    "pfireworksm", "pfireworksn", "pfireworkso", "pfireworksp", "pfireworksq", "pfireworksr",
    "pfireworkss", "pfireworkst", "pfireworksy",
}

cfg.escrowPazeeeKeys = cfgRP.escrowPazeeeKeys
do
    local escrowMod = CMG.loadModule("cfg/cfg_pazeee_emotes")
    if escrowMod and escrowMod.keys then
        cfg.escrowPazeeeKeys = escrowMod.keys
    end
end

do
    local skipStandard = {}
    for _, c in ipairs(cfg.fireworksEmotes) do
        skipStandard[c] = true
    end
    if cfg.escrowPazeeeKeys and cfg.escrowPazeeeKeys.all then
        for _, c in ipairs(cfg.escrowPazeeeKeys.all) do
            skipStandard[c] = true
        end
    end
    cfg.standardEmotes = {}
    for name, _ in pairs(cfg.emotes) do
        if not skipStandard[name] then
            cfg.standardEmotes[#cfg.standardEmotes + 1] = name
        end
    end
    table.sort(cfg.standardEmotes)
end
