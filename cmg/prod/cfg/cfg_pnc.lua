--[[
    LEVEL 1 BEGINNER GUIDE — Pnc
    =================================

    File: cmg/prod/cfg/cfg_pnc.lua
...
cfg.offences = {
    ...
    ["forgery"] = {
        ...
        punishments = {
            {fine = 10000, months = 0, points = 0},
            {fine = 20000, months = 0, points = 0},
            {fine = 30000, months = 0, points = 0}
        }
    }
}

-- Halved prison sentences
for _, offence in pairs(cfg.offences) do
    for _, punishment in ipairs(offence.punishments) do
        if punishment.months > 0 then
            punishment.months = math.floor(punishment.months / 2)
        end
    end
end
